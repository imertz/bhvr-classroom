...
export class TeacherRepo extends Context.Service<TeacherRepo, {
  ...
  readonly create: (input: TeacherInput) => Effect.Effect<Teacher, DatabaseError>;
  readonly update: (id: string, input: Partial<TeacherInput>) => Effect.Effect<Teacher, NotFoundError | DatabaseError>;
  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function*() {
      ...
      const create = Effect.fn("TeacherRepo.create")(function*(input: TeacherInput) {
        ...
        const res = yield* sqlite.queryOne<Teacher>(
      ...
      const update = Effect.fn("TeacherRepo.update")(function*(id: string, input: Partial<TeacherInput>) {
        ...
        const res = yield* sqlite.queryOne<Teacher>(updateQuery, params);
        if (!res) {
          return yield* new NotFoundError({ message: `Teacher with id ${id} not found`, entity: "Teacher", id });
        }
        return res;
