...
export class SqliteClient extends Context.Service<SqliteClient, {
  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function*() {
      ...
      let isClosed = false;
      const closeDb = (dbInstance: Database) => {
        if (isClosed) return;
        isClosed = true;
        try {
          ...
          // Ignore if already closed or during exit
        }
      };
      ...
      const db = yield* Effect.acquireRelease(
        Effect.sync(() => {
          ...
          dbInstance.fileControl(constants.SQLITE_FCNTL_PERSIST_WAL, 0);
          ...
          if (!dbExists) {
            ...
            const teachersInfo = dbInstance.query("PRAGMA table_info(teachers)").all() as TableColumnInfo[];
            if (!teachersInfo.some((c) => c.name === "role")) {
              dbInstance.exec("ALTER TABLE teachers ADD COLUMN role TEXT DEFAULT 'teacher' CHECK (role IN ('teacher', 'admin'))");
              dbInstance.exec("CREATE INDEX IF NOT EXISTS idx_teachers_role ON teachers(role)");
            }
            // SAFETY: PRAGMA table_info returns column metadata list
            const studentsInfo = dbInstance.query("PRAGMA table_info(students)").all() as TableColumnInfo[];
            if (!studentsInfo.some((c) => c.name === "role")) {
              dbInstance.exec("ALTER TABLE students ADD COLUMN role TEXT DEFAULT 'student' CHECK (role IN ('student'))");
            }
          }

          process.on("exit", () => closeDb(dbInstance));

          return dbInstance;
        }),
        (dbInstance) => Effect.sync(() => closeDb(dbInstance))
      );
